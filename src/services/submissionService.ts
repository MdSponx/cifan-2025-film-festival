...
export class SubmissionService {
  ...
  /**
   * Saves a youth form as draft (without file requirements)
   */
  async saveDraftYouthForm(formData: YouthFormData): Promise<SubmissionResult> {
    try {
      ...
      // Stage 2: Skip file upload for drafts
      this.updateProgress('saving', 50, 'Saving draft...');

      // Stage 3: Save to Firestore without files
      const docRef = await this.saveDraftToFirestore('youth', formData);
  ...
  /**
   * Saves a future form as draft (without file requirements)
   */
  async saveDraftFutureForm(formData: FutureFormData): Promise<SubmissionResult> {
    try {
      ...
      this.updateProgress('saving', 50, 'Saving draft...');
      const docRef = await this.saveDraftToFirestore('future', formData);
  ...
  /**
   * Saves draft submission data to Firestore (without files)
   */
